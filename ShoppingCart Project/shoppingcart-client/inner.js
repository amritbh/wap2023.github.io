    ...
    products.forEach((product) => {
      ...
      addButtonElement.addEventListener("click", () => {
        ...
        let cartFromStorage = localStorage.getItem("cart")
          ...
          : [];
        const itemIndex = cartFromStorage.findIndex(
      ...
      for (let i = 0; i < cart.length; i++) {
        ...
        minusButton.addEventListener("click", function () {
          let product = products.find(
            (product) => product.name === cart[i].name
          );
        ...
        plusButton.addEventListener("click", function () {
          ...
          if (cart[i].quantity<product.stock_quantity) {
            cart[i].quantity++;
          }else if(cart[i].quantity===product.stock_quantity){
            alert("Stock Quantity exceeded");
          }
          localStorage.setItem("cart", JSON.stringify(cart));
          window.location.reload();
